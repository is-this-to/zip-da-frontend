...
export const deposit = (val, txType) => {
  ...
  if (!val || val <= 0)
    return txType === "JEONSE" ? "전세 보증금을 입력해 주세요." : "보증금을 입력해 주세요.";
  return "";
};
...
export const monthlyRent = (val, txType) => {
  ...
  if (!val || val <= 0)
    return txType === "MONTHLY_RENT" ? "월세 금액을 입력해 주세요." : "임대료를 입력해 주세요.";
  return "";
};

export const areaM2 = (val) => {
  if (val === null || val === undefined || val === "") return "";
  if (val <= 0 || val >= 10000) return "전용면적은 0보다 크고 10000보다 작아야 합니다.";
  return "";
};
